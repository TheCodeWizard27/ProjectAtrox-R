...
func _mouse_input_event(_camera: Camera3D, event: InputEvent, event_position: Vector3, _normal: Vector3, _shape_idx: int):
	...
	var screen_size = display_mesh.mesh.size
	...
	var screen_pos = display_mesh.global_transform.affine_inverse() * event_position
	...
	if _is_mouse_inside:
		...
		local_pos.x *= sub_viewport.size.x
		local_pos.y *= sub_viewport.size.y
