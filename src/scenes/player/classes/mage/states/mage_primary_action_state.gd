...
class_name MagePrimaryActionState
extends MageState

class PrimaryAttackData:
	var blend_position: MageModelAnimator.ActionBlendPosition
	var effect_step_type: MageEventPlayer.PrimaryAttackSteps
	var cancel_window_start: float
	var max_duration: float
	
	func _init(
		p_effect_step_type: MageEventPlayer.PrimaryAttackSteps,
		p_blend_position: MageModelAnimator.ActionBlendPosition, 
...
var attack_steps: Array[PrimaryAttackData] = [
	PrimaryAttackData.new(
		MageEventPlayer.PrimaryAttackSteps.STEP_1,
		MageModelAnimator.ActionBlendPosition.PRIMARY_ATTACK_1,
		0.2,
		0.45
	),
	PrimaryAttackData.new(
		MageEventPlayer.PrimaryAttackSteps.STEP_2,
		MageModelAnimator.ActionBlendPosition.PRIMARY_ATTACK_2,
		0.2,
		0.41
	),
	PrimaryAttackData.new(
		MageEventPlayer.PrimaryAttackSteps.STEP_3,
		MageModelAnimator.ActionBlendPosition.PRIMARY_ATTACK_3,
		0.2,
		0.625
	)
]
...
func process_attack(delta: float) -> StateResult:
	...
	if (step_end_reached):
		...
		return StateResult.transition_to(MageState.GROUNDED)
...
func _init_attack_step(step: int) -> void:
	...
	model_animator.mage_animator.play_action(current_step.blend_position)
	event_player.mage_event_player.play_primary_effect(current_step.effect_step_type)
